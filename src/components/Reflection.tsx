        ...
        <div className="flex justify-center gap-2">
          {moods.map(([mood, config], i) => (
            <motion.button
              ...
              className={`flex flex-col items-center gap-1 p-2.5 rounded-2xl transition-all min-w-[56px] ${
                selected === mood
      ...
      {selected && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-3">
          <textarea
            ...
            className="w-full glass rounded-2xl p-3 text-sm resize-none h-20 focus:outline-none focus:ring-2 focus:ring-primary/30 placeholder:text-muted-foreground/60"
          />
          <motion.button
            whileTap={{ scale: 0.97 }}
            onClick={() => onSubmit(selected, note || undefined)}
            className="w-full py-3 rounded-2xl gradient-calm text-primary-foreground font-medium shadow-glow"
          >
            Save & Continue
          </motion.button>
        </motion.div>
